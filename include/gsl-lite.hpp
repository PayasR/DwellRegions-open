...
#if gsl_BETWEEN(gsl_COMPILER_GNUC_VERSION, 1, 500) || gsl_BETWEEN(gsl_COMPILER_CLANG_VERSION, 1, 360) ||               \
...
#define gsl_DEFINE_ENUM_BITMASK_OPERATORS_(ENUM)                                                                       \
  ...
  gsl_api inline gsl_constexpr14 ENUM& operator|=(ENUM& lhs, ENUM rhs) gsl_noexcept { return lhs = lhs | rhs; }        \
  gsl_api inline gsl_constexpr14 ENUM& operator&=(ENUM& lhs, ENUM rhs) gsl_noexcept { return lhs = lhs & rhs; }        \
  gsl_api inline gsl_constexpr14 ENUM& operator^=(ENUM& lhs, ENUM rhs) gsl_noexcept { return lhs = lhs ^ rhs; }
...
#if !gsl_BETWEEN(                                                                                                      \
  gsl_COMPILER_MSVC_VERSION, 1, 140) // VS 2013 seems to have trouble with SFINAE for default non-type arguments
...
// add_const<>, add_pointer<>, common_type<>, make_signed<>, remove_cv<>, remove_const<>, remove_volatile<>,
// remove_reference<>, remove_cvref<>, remove_pointer<>, underlying_type<>, is_assignable<>, is_constructible<>,
// is_const<>, is_convertible<>, is_integral<>, is_pointer<>, is_signed<>, integral_constant<>, declval()
  ...
  namespace gsl
{
  ...
  struct has_size_and_data<
    C,
    std17::void_t<decltype(std17::size(std::declval<C>())), decltype(std17::data(std::declval<C>()))>> : std::true_type
  {};
  ...
  template<class C, class E>
  struct is_compatible_container : std17::bool_constant<is_container<C>::value && is_compatible_element<C, E>::value>
  {};
...
#define gsl_CONTRACT_CHECK_(str, x)                                                                                    \
  ((x) ? static_cast<void>(0) : ::gsl::fail_fast_assert_handler(#x, "GSL: " str, __FILE__, __LINE__))
#elif defined(__CUDACC__) && defined(__CUDA_ARCH__)
#define gsl_CONTRACT_CHECK_(str, x) assert((x) && str)
#elif defined(gsl_CONFIG_CONTRACT_VIOLATION_THROWS)
#define gsl_CONTRACT_CHECK_(str, x)                                                                                    \
  ((x) ? static_cast<void>(0) : ::gsl::detail::fail_fast_throw("GSL: " str " at " __FILE__ ":" gsl_STRINGIFY(__LINE__)))
...
#define gsl_STRINGIFY_(x) #x
  ...
  gsl_DEPRECATED_MSG("don't call gsl::fail_fast_assert() directly; use contract checking macros instead")
    ...
    fail_fast_assert(
      bool cond, char const* const expression, char const* const message, char const* const file, int line)
  {
    if (!cond)
  ...
  gsl_api inline unsigned char to_uchar(unsigned x) gsl_noexcept { return static_cast<unsigned char>(x); }
  ...
  inline unsigned char uncaught_exceptions() gsl_noexcept { return detail::to_uchar(std::uncaught_exceptions()); }
    ...
    narrow(U u)
  {
    ...
    gsl_SUPPRESS_MSVC_WARNING(4127,
                              "conditional expression is constant") if (!detail::is_same_signedness<T, U>::value &&
                                                                        (t < T()) != (u < U()))
#endif
#else
...
#else
    // Don't assume T() works:
    gsl_SUPPRESS_MSVC_WARNING(4127, "conditional expression is constant") gsl_Expects((t < 0) == (u < 0));
#endif
...
// helper class to figure out the pointed-to type of a pointer
...
#if gsl_HAVE(TYPE_TRAITS) && gsl_HAVE(DEFAULT_FUNCTION_TEMPLATE_ARG) &&                                                \
  !gsl_BETWEEN(gsl_COMPILER_CLANG_VERSION, 1, 400) && !gsl_BETWEEN(gsl_COMPILER_APPLECLANG_VERSION, 1, 1001)
  ...
  !gsl_BETWEEN(gsl_COMPILER_CLANG_VERSION, 1, 400) && !gsl_BETWEEN(gsl_COMPILER_APPLECLANG_VERSION, 1, 1001)
    ...
    template<class U
             // We *have* to use SFINAE with an NTTP arg here, otherwise the overload is ambiguous.
             ,
             typename std::enable_if<(std::is_constructible<U, T const&>::value && !std::is_convertible<T, U>::value &&
    ...
    template<class U
             // We *have* to use SFINAE with an NTTP arg here, otherwise the overload is ambiguous.
             ,
             typename std::enable_if<(std::is_constructible<U, T>::value && !std::is_convertible<T, U>::value &&
                                      !detail::is_not_null_oracle<U>::value),
                                     int>::type = 0>
    ...
    template<class U
             // We *have* to use SFINAE with an NTTP arg here, otherwise the overload is ambiguous.
             ,
             typename std::enable_if<(std::is_convertible<T, U>::value && !detail::is_not_null_oracle<U>::value),
                                     int>::type = 0>
  ...
  template<class T, class U>
  inline gsl_constexpr gsl_DECLTYPE_(bool, std::declval<T const>() == std::declval<U const>()) operator==(
  ...
  template<class T, class U>
  inline gsl_constexpr gsl_DECLTYPE_(bool, std::declval<T const>() == std::declval<U const>()) operator==(
    not_null<T> const& l, U const& r)
  {
    return l.operator->() == r;
  }
  template<class T, class U>
  inline gsl_constexpr gsl_DECLTYPE_(bool, std::declval<T const>() == std::declval<U const>()) operator==(
    T const& l, not_null<U> const& r)
  {
    return l == r.operator->();
  }
  ...
  template<class T, class U>
  inline gsl_constexpr gsl_DECLTYPE_(bool, !(std::declval<U const>() < std::declval<T const>())) operator<=(
    not_null<T> const& l, not_null<U> const& r)
  {
    return !(r < l);
  }
...
//
  ...
  gsl_api inline gsl_constexpr byte operator|(byte l, byte r) gsl_noexcept { return ::gsl::to_byte(l.v | r.v); }
  ...
  const gsl_constexpr with_container_t with_container; // TODO: this can lead to ODR violations because the symbol will
  ...
  public:
    ...
    {}
#else
    template<size_t N gsl_ENABLE_IF_((std::is_convertible<value_type (*)[], element_type (*)[]>::value))>
    ...
    template<class Container gsl_ENABLE_IF_((detail::is_compatible_container<Container, element_type>::value))>
    gsl_constexpr span(Container& cont) gsl_noexcept
      : first_(std17::data(cont))
      , last_(std17::data(cont) + std17::size(cont))
    {}

    template<class Container gsl_ENABLE_IF_((std::is_const<element_type>::value &&
    ...
    gsl_DEPRECATED_MSG("use subscript indexing instead") gsl_api gsl_constexpr14 reference operator()(
      index_type pos) const
    {
      return (*this)[pos];
    }

    gsl_DEPRECATED_MSG("use subscript indexing instead") gsl_api gsl_constexpr14 reference at(index_type pos) const
    {
      return (*this)[pos];
    }
#endif // deprecate
    ...
    gsl_constexpr17 const_reverse_iterator crbegin() const gsl_noexcept { return const_reverse_iterator(cend()); }

    gsl_constexpr17 const_reverse_iterator crend() const gsl_noexcept { return const_reverse_iterator(cbegin()); }
    ...
    gsl_DEPRECATED_MSG("use size() instead") gsl_api gsl_constexpr index_type length() const gsl_noexcept
    {
      return size();
    }
    ...
    gsl_DEPRECATED_MSG("use size_bytes() instead") gsl_api gsl_constexpr index_type length_bytes() const gsl_noexcept
    {
      return size_bytes();
    }
#endif
    ...
    gsl_DEPRECATED_MSG("use free function gsl::as_bytes() instead") gsl_api span<const byte> as_bytes() const
      gsl_noexcept
    {
    ...
    gsl_DEPRECATED_MSG(
      "use free function gsl::as_writable_bytes() instead") gsl_api span<byte> as_writeable_bytes() const gsl_noexcept
    {
...
#if !gsl_DEPRECATE_TO_LEVEL(6)
  template<class T>
  gsl_DEPRECATED_MSG("use as_writable_bytes() (different spelling) instead") gsl_api inline span<byte>
  ...
  inline gsl_constexpr auto make_span(Container & cont)->span<typename std::remove_pointer<EP>::type>
  {
    return span<typename std::remove_pointer<EP>::type>(cont);
  }

  template<class Container, class EP = decltype(std17::data(std::declval<Container&>()))>
  inline gsl_constexpr auto make_span(Container const& cont)->span<const typename std::remove_pointer<EP>::type>
  {
  ...
  template<class Container>
  inline gsl_constexpr span<typename Container::value_type> make_span(with_container_t, Container & cont) gsl_noexcept
  {
    return span<typename Container::value_type>(with_container, cont);
  }

  template<class Container>
  inline gsl_constexpr span<const typename Container::value_type> make_span(with_container_t, Container const& cont)
    gsl_noexcept
  {
  ...
  public:
...
#if gsl_HAVE(NULLPTR)
    gsl_api gsl_constexpr basic_string_span(std::nullptr_t) gsl_noexcept : span_(nullptr, static_cast<index_type>(0)) {}
#endif
    ...
    template<class U gsl_ENABLE_IF_((std::is_convertible<typename basic_string_span<U>::pointer, pointer>::value))>
    ...
    /*gsl_api*/ // currently disabled due to an apparent NVCC bug
    gsl_constexpr14 basic_string_span subspan(index_type offset) const { return span_.subspan(offset); }
  ...
  template<class T, class U>
  gsl_SUPPRESS_MSGSL_WARNING(stl .1) inline gsl_constexpr14 bool operator==(basic_string_span<T> const& l, U const& u)
    gsl_noexcept
  {
  ...
  gsl_SUPPRESS_MSGSL_WARNING(stl .1) inline gsl_constexpr14 bool operator==(U const& u, basic_string_span<T> const& r)
    gsl_noexcept
  {
  ...
  template<class T>
  gsl_SUPPRESS_MSGSL_WARNING(stl .1) inline gsl_constexpr14 bool operator==(basic_string_span<T> const& l,
...
inline std::basic_string< typename std::remove_const<T>::type > to_string( basic_string_span<T> spn )
{
  ...
  inline std::string to_string(string_span const& spn) { return std::string(spn.data(), spn.length()); }

  inline std::string to_string(cstring_span const& spn) { return std::string(spn.data(), spn.length()); }

#if gsl_HAVE(WCHAR)

  inline std::wstring to_string(wstring_span const& spn) { return std::wstring(spn.data(), spn.length()); }

  inline std::wstring to_string(cwstring_span const& spn) { return std::wstring(spn.data(), spn.length()); }
  ...
  template<class Container>
  inline span<typename std::remove_pointer<typename Container::pointer>::type> ensure_z(Container & cont)
  {
  ...
  public:
    ...
    /*gsl_api*/ // currently disabled due to an apparent NVCC bug
    gsl_constexpr string_span_type ensure_z() const { return ::gsl::ensure_z(span_.data(), span_.size()); }
...
{
public:
  gsl_constexpr std::size_t operator()(::gsl::byte v) const gsl_noexcept { return ::gsl::to_integer<std::size_t>(v); }
};
